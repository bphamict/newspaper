...
module.exports = {
    ...
    searchPosts: async (searchStr, searchBy = 'title', limit, offset, isAuthenticated) => {
        ...
        const rows = await db.load(`SELECT P.*, C.name as category_name, SC.name as sub_cagegory_name FROM ${TBL} P JOIN CATEGORY C ON P.category_id = C.id JOIN SUB_CATEGORY SC ON P.sub_category_id = SC.id WHERE MATCH(${searchBy}) AGAINST ('"${searchStr}"' IN BOOLEAN MODE) AND (status = 'PUBLISHED' OR (status = 'APPROVED' AND publish_time <= NOW())) ORDER BY ${orderIfAuthenticated} publish_time DESC LIMIT ${offset}, ${limit}`);
