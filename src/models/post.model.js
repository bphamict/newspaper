...
module.exports = {
    ...
    loadBySlugWithCategoryAndSubCategoryName: async (slug) => {
        const rows = await db.load(`SELECT P.*, C.name AS category_name, C.slug AS category_slug, SC.name AS sub_category_name, SC.slug as sub_category_slug FROM ${TBL} P JOIN CATEGORY C ON P.category_id = C.id JOIN SUB_CATEGORY SC ON P.sub_category_id = SC.id WHERE P.slug = '${slug}' AND P.isDeleted != 1`);
    ...
    searchPosts: async (searchStr, searchBy = 'title', limit, offset, isAuthenticated) => {
        ...
        const rows = await db.load(`SELECT P.*, C.name as category_name, C.slug as category_slug, SC.name as sub_category_name, sc.slug as sub_category_slug FROM ${TBL} P JOIN CATEGORY C ON P.category_id = C.id JOIN SUB_CATEGORY SC ON P.sub_category_id = SC.id WHERE MATCH(${searchBy}) AGAINST ('"${searchStr}"' IN BOOLEAN MODE) AND P.isDeleted != 1 AND (status = 'PUBLISHED' OR (status = 'APPROVED' AND publish_time <= NOW())) ORDER BY ${orderIfAuthenticated} publish_time DESC LIMIT ${offset}, ${limit}`);
    ...
    pageBySlugCategory: async (slug, limit, offset, isAuthenticated) => {
        ...
        const rows = await db.load(`SELECT p.*, c.name as category_name, c.slug as category_slug, sc.name as sub_category_name, sc.slug as sub_category_slug FROM ${TBL} p join category c ON p.category_id = c.id JOIN SUB_CATEGORY sc ON p.sub_category_id = sc.id WHERE p.isDeleted != 1 AND c.slug = '${slug}' AND (p.status = 'PUBLISHED' OR (p.status = 'APPROVED' AND p.publish_time <= NOW())) ORDER BY ${orderIfAuthenticated} p.publish_time DESC limit ${limit} offset ${offset}`);
    ...
    countBySlugCategory: async (slug) => {
        const row = await db.load(`SELECT count(*) as total FROM ${TBL} p join category c ON p.category_id = c.id WHERE p.isDeleted != 1 AND c.slug = '${slug}' AND (p.status = 'PUBLISHED' OR (p.status = 'APPROVED' AND p.publish_time <= NOW()))`);
    ...
    pageBySlugSubCategory: async (slug, limit, offset, isAuthenticated) => {
        ...
        const rows = await db.load(`SELECT p.*, c.name as category_name, c.slug as category_slug, sc.name as sub_category_name, sc.slug as sub_category_slug FROM ${TBL} p join category c ON p.category_id = c.id JOIN SUB_CATEGORY sc ON p.sub_category_id = sc.id WHERE p.isDeleted != 1 AND sc.slug = '${slug}' AND (p.status = 'PUBLISHED' OR (p.status = 'APPROVED' AND p.publish_time <= NOW())) ORDER BY ${orderIfAuthenticated} p.publish_time DESC limit ${limit} offset ${offset}`);
    ...
    countBySlugSubCategory: async (slug) => {
        const row = await db.load(`SELECT count(*) as total FROM ${TBL} p join sub_category sc ON p.sub_category_id = sc.id WHERE p.isDeleted != 1 AND sc.slug = '${slug}' AND (p.status = 'PUBLISHED' OR (p.status = 'APPROVED' AND p.publish_time <= NOW()))`);
