...
router.post('/:id/details', isAdmin, async (req, res) => {
    ...
    if(role_id === 4) {
        await Promise.all([userSubscribeModel.delete(userID), userSubscribeModel.add({ user_id: userID, expiry_time: req.body.expiry_time }), categoryModel.deleteUser({ userID })]);
    } else if (role_id === 3) {
        await Promise.all([userSubscribeModel.delete(userID), categoryModel.deleteUser({ userID })]);
    } else {
